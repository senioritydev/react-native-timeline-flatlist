...
export default class Timeline extends PureComponent {
  ...
  _renderDetail(rowData, rowID) {
    const { isAllowFontScaling, titleStyle,trackingTimeStyle,detailTitleStyle,detailSubTitleStyle} = this.props;
    const {orderTrackingData} = rowData
    let description;
    ...
    return (
      ...
      >
        ...
        <Text
          style={rowData.enableTracking ? titleStyle : detailSubTitleStyle}
          allowFontScaling={isAllowFontScaling}
        >
          {rowData.title}
          </Text>
          <Text
          style={trackingTimeStyle}
          allowFontScaling={isAllowFontScaling}
        >
          {rowData.orderTime}
          </Text>
          </View>
        {orderTrackingData.map(item => <View style = {{marginTop : 7,marginBottom : 18}}>
          <Text style = {rowData.enableTracking ? detailTitleStyle :detailSubTitleStyle } >{item.title}</Text>
          <Text style = {detailSubTitleStyle }>{item.content}</Text>
        </View>)}
      </View>
    );
  }
